...
#define MAX_DIAGNOSTIC_PAYLOAD_SIZE 2047
                                     ...
                                     DiagnosticShims *shims)
{
    ...
    if (response->mode == handle->request.mode + MODE_RESPONSE_OFFSET)
    {
        ...
        if ((!handle->request.has_pid && !response->has_pid) || response->pid == handle->request.pid)
        {
                                                ...
                                                const uint8_t data[], const uint8_t size)
{
    ...
    else
    {
        ...
        for (i = 0; i < handle->isotp_receive_handle_count; ++i)
        {
            ...
            { // NEW 4/25/2020- Copy partial message into the response buffer
